    ...
    class Ship:Obj
    {
        ...
        public override void Update(GameTime gameTime)
        {
            if (IsActive && IsReady)
            {
                ...
                if (GameplayScreen.keyboardState.IsKeyDown(controls[(int)Controls.Up]) && Speed < 10)     /* Augmente le Speed */
                    Speed += 0.5F;
                if (GameplayScreen.keyboardState.IsKeyDown(controls[(int)Controls.Down]) && Speed > -10)   /* Diminue le Speed */
                    Speed -= 0.5F;
                if (GameplayScreen.keyboardState.IsKeyDown(controls[(int)Controls.Left]))   /* Modifie la Direction */
                    Degrees -= 6;
                if (GameplayScreen.keyboardState.IsKeyDown(controls[(int)Controls.Right]))  /* Modifie la Direction */
                    Degrees += 6;

                if (GameplayScreen.keyboardState.IsKeyDown(controls[(int)Controls.Fire]) && GameplayScreen.oldKeyboardState.IsKeyUp(controls[(int)Controls.Fire])) // On fait un tir à chaque nouvel appui sur la barre espace
                    GameplayScreen.waitingComponents.Add(new Bullet(HitBox[0] + Position, Direction, this));
            }
        }
        ...
        public override void Draw(GameTime gameTime)
        {
            if (IsActive && IsReady)
            {
                GameplayScreen.spriteBatch.Draw(
                ...
                GameplayScreen.spriteBatch.DrawString(
                    GameplayScreen.font,                                 /* SpriteFont spriteFont */
                    Pseudo,                                             /* string text */
                    Position - new Vector2(GameplayScreen.font.MeasureString(Pseudo).X * 0.3f / 2, -(Image.Height / 2 + GameplayScreen.font.MeasureString(Pseudo).Y * 0.3f)), /* Vector2 position */
                    GameplayScreen.teamColor[Team],                                        /* Color color */
                    0f,                                                 /* float rotation */
        ...
        {
            int size = 30;
            Image = new Texture2D(GameplayScreen.game.GraphicsDevice, size, size, false, SurfaceFormat.Color);
            ...
            {
                colorTable[size / 2 - i + size * (i * 2)] = GameplayScreen.teamColor[Team];
                hitBox[4 * i] = new Vector2((-i), (i * 2) - size / 2);
                colorTable[size / 2 - i + size * (2 * i + 1)] = GameplayScreen.teamColor[Team];
