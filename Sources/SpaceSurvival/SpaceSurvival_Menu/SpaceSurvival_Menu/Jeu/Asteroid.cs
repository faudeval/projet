...
namespace SpaceSurvival
{
    class Asteroid:Obj
    {
        ...
        {
            Texture2D texture = new Texture2D(GameplayScreen.game.GraphicsDevice, (value * 20), (value * 20), false, SurfaceFormat.Color);
        ...
        Vector2 generatePosition(int x = 0, int y = 0, int width = -1, int height = -1)
        {
            ...
            if (width <= 0 || height <= 0)
            {
                width = GameplayScreen.game.Window.ClientBounds.Width;
                height = GameplayScreen.game.Window.ClientBounds.Height;
            }
